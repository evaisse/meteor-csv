...
Tinytest.add('test csv read se fibers to grab all lines in synchronous way', function (test) {
    ...
    CSV.readCsvFileLineByLine(process.env.PWD + '/./fixtures/test-4-rows.csv', function (line) {
        lines.push(line);
    });
...
Tinytest.add('test csv file is readed line by line', function (test) {
    ...
    CSV.readCsvFileLineByLine(process.env.PWD + '/./fixtures/test-headers-35-rows.csv', { headers: true }, function (line) {
        lastLine = line;
        linesCount++;
    });
...
Tinytest.add('test customs delimiter and vicious formats', function (test) {
    ...
    CSV.readCsvFileLineByLine(process.env.PWD + '/./fixtures/test-custom-delim.csv', { 
        headers: true,
        delimiter: ":",
    }, function (line) {
        lastLine = line;
